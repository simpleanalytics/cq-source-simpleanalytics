...
import (
	...
	"github.com/simpleanalytics/cq-source-simple-analytics/resources/services"
...
type Client struct {
	...
	services    *simpleanalytics.Client
...
func Configure(_ context.Context, logger zerolog.Logger, specBytes []byte, opts plugin.NewClientOptions) (plugin.Client, error) {
	if opts.NoConnection {
		return &Client{
			logger:  logger.With().Str("module", "simpleanalytics").Logger(),
			options: opts,
			tables:  getTables(),
		}, nil
	}
	...
	saClient := simpleanalytics.NewClient(config.UserID, config.APIKey)
